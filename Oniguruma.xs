...
Oniguruma_comp(pTHX_ const SV * const pattern, const U32 flags)
{
    ...
    if (ri == NULL) {
        croak("Oniguruma compilation failed at offset %d: %s\n", erroffset, error);
        return NULL;
    }
...
char *
Oniguruma_intuit(pTHX_ REGEXP * const rx, SV * sv, char *strpos,
...
void
Oniguruma_make_nametable(REGEXP * const rx, pcre * const ri, const int namecount)
{
